...
(* MARCUS: load from uncompiled .v files instead of .vo *)
Add LoadPath "/Users/marcus/coq_int".
Load Axioms.
Load Coqlib.
(*
...
Definition repr (x: Z) : int := 
  mkint (intval:=Zmod x modulus)(Z_mod_lt x modulus modulus_pos).
...
Lemma mkint_eq:
  forall x y Px Py, x = y -> mkint (intval:=x) Px = mkint (intval:=y) Py.
Proof.
  intros. subst y. 
...
(* Examples are below. *)
(* BTW:
1.  Locate is really useful for finding where an object, symbol, etc. is.
2. Locate Int.* refers to the 32-bit integer type, as a predefined shortcut.
3. Once you store something into Int, the library does not "know" whether it is
signed or unsigned.
4. To see the value of an Int, you have to use the "compute" tactic instead of "simpl"
*)


Locate repr.
Check Int.repr.
Definition x := Int.repr 5.
Definition y := Int.repr 12.

Eval compute in Int.add x y.

Definition w := Int.repr (Z.neg 7).

Definition v := Int.add x w.



Eval compute in Int.signed v.
